...
private val TauriConfigTypeName = ClassName("revxrsal.kitsune.app", "TauriConfig")

/** Discovered by KSP through `META-INF/services`; must be public with a no-arg constructor. */
class StaticFilesProcessorProvider : SymbolProcessorProvider {
    override fun create(environment: SymbolProcessorEnvironment): SymbolProcessor {
        return StaticFilesProcessor(environment.codeGenerator, environment.options, environment.logger)
    }
}

/** Absolute path of the file to write. Set from `kitsune.tauriConfig` in the build. */
const val CONFIG_OPTION = "kitsune.tauriConfig"
private const val FILE_NAME = "GeneratedTauriConfig"

private val TJson = Json { ignoreUnknownKeys = true }

/**
 * Reads `tauri.config.json` and parses it to a Kotlin data
 * object
 */
class StaticFilesProcessor(
    ...
    /** Blank means the build did not configure `kitsune.tauriConfig`; nothing is generated. */
    private val tauriConfigPath = options[CONFIG_OPTION]?.takeIf { it.isNotBlank() }
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        val path = File(tauriConfigPath ?: run {
            logger.info("Kitsune: no '$CONFIG_OPTION' option; skipping the Tauri config parser.")
            return emptyList()
        })

        val config = TJson.decodeFromString<TauriConfigJson>(path.readText())
        val generated = TypeSpec.objectBuilder(FILE_NAME)
            .addKdoc("Parses the tauri.config.json file and provides its values directly for Kotlin consumption")

        generated.addProperty(
            PropertySpec.builder("config", TauriConfigTypeName)
                .addKdoc("The literals read directly from tauri.conf.json")
